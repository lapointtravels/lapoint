...
require_once(dirname(__FILE__) .'/modules/theme-module.php');
...
class Kloon_Module_Controller {
	...
	public function __construct () {
		...
		add_action('wp_ajax_get_empty_tinymce_editor', array($this, 'ajax_get_empty_tinymce_editor'));
	...
	public function after_setup () {
		// Fetch settings
		$this->_settings = apply_filters('kmc/settings', array());

		// Register custom theme modules
		$theme_modules = apply_filters('kmc/register_kmc_theme_modules', array());
		foreach ($theme_modules as $theme_module) {
			new Theme_Module((object) $theme_module);
		}

		// Make the data global since we need it
		$this->theme_modules = $theme_modules;

		$this->animation_types = apply_filters('kmc/register_kmc_animations', array());
	...
	public function admin_notice () {
		...
		foreach ($KMC_MODULES as $module) {
			foreach ($module->requires as $key => $value) {
				if ($key == "module-controller") {
					if ($this->version < $value) {
						$this->show_dependency_box($module, $value, $key);
					}
				} else {
					if (isset($KMC_MODULES[$key])) {
						if ($KMC_MODULES[$key]->version < $value) {
							$this->show_dependency_box($module, $value, $key);
						}
					} else {
						$this->show_dependency_box($module, $value, $key);
	...
	private function show_dependency_box ($module, $version, $dependency) {
		echo '<div class="notice notice-error"><p>';
		echo $module->name . " requires at least version " . $version . " of " . $dependency;
		echo '</p></div>';
	}
	...
	public function load_localization () {
		load_plugin_textdomain('kmc', false, basename( dirname( __FILE__ ) ) . '/languages');
	}
	...
	*/
	public function kmc_meta_box_add () {
		...
		add_meta_box('kmc-meta-box', __("Page composer", "kmc") , array($this, 'kmc_meta_box'), $admin_types, 'normal', 'high');
	}
	...
	*/
	...
	function add_admin_body_class($classes) {
		...
		if ($screen->post_type == "page") {
			...
			if ($has_kmc) {
				...
				add_meta_box('kmc-meta-box', __("Page composer", "kmc"), array($this, 'kmc_meta_box'), array('page'), 'normal', 'high');
			}
	...
	public function kmc_meta_box () {
		...
		wp_enqueue_script('kmc-theme-module', plugins_url('js/theme-component.js', __FILE__));

		do_action("kcm/enqueue_admin_scripts");

		wp_enqueue_media();
		wp_enqueue_script('kmc-admin', plugins_url('js/admin.js', __FILE__), array(), $this->version);
		wp_enqueue_script('jscolor', plugins_url('js/jscolor.min.js', __FILE__));
		wp_enqueue_style('kmc-admin', plugins_url('css/admin.css', __FILE__), array(), $this->version);
			...
			*/
			...
			foreach ($sections as $section) {
				...
				foreach ($section->components as $component) {
					if (!$component->saved) {
						if (!isset($component->post->ID)) {
							$new_component_id = $KMC_MODULES[$component->type]->create_instance($component, $is_preview);

							// Call update on newly created objects as well, to handle custom data
	...
	public function get_page_data ($page_id, $is_preview) {
		...
		if ($kmc_page_components_meta) {
			...
			foreach ($meta_json as $section_json) {
				...
				// Set missing settings to false
				if (!property_exists($section->settings, 'name')) {
					$section->settings->name = false;
				}
				if (!property_exists($section->settings, 'background_image')) {
					$section->settings->background_image = false;
				}
				if (!property_exists($section->settings, 'background_video_ogv')) {
					$section->settings->background_video_ogv = false;
				}
				if (!property_exists($section->settings, 'background_video_mp4')) {
					$section->settings->background_video_mp4 = false;
				}
	...
	public function get_component ($post_id, $is_preview=false) {
		...
		if ($post && $KMC_MODULES[$post->post_type]) {
			return $KMC_MODULES[$post->post_type]->create_instance_from_post($post);
		} else {
			return false;
		}
	}
	...
	public function ajax_get_empty_tinymce_editor () {
		$id = $_POST["id"];
		wp_editor("", $id);
		exit;
	}
